...
from algos.multi.data_gen import Seq2PointMultiDataGenerator
...
def get_train_data(df_train, df_val, features, ref_norm, seq_len):
    ...
    train_data_generator = Seq2PointMultiDataGenerator(x_train, y_train, seq_len=seq_len)
    val_data_generator = Seq2PointMultiDataGenerator(x_val, y_val, seq_len=seq_len)
...
def get_test_data(df_test, features, ref_norm, seq_len):
    ...
    test_data_generator = Seq2PointMultiDataGenerator(x_test, y_test, seq_len=seq_len)
    x_test, y_test = test_data_generator.load_all()

    return x_test, y_test

def train(model, features, df_train, df_val, ref_norm, seq_len, seq_per_batch, epochs, checkpoint_path):
    x_train, y_train, x_val, y_val = get_train_data(
        df_train=df_train, df_val=df_val, features=features, ref_norm=ref_norm, seq_len=seq_len)
...
def predict(model, features, df_test, ref_norm, seq_len, seq_per_batch):
    x_test, y_test = get_test_data(df_test, features=features, ref_norm=ref_norm, seq_len=seq_len)
