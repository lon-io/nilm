...
def create_model(seq_len, learning_rate=1e-1):
    ...
    model.add(Conv1D(3, 4, activation="linear", padding="same", strides=1))
...
def get_train_data(df_train, df_val, features, ref_norm, seq_len, seq_per_batch):
...
def get_test_data(df_test, features, ref_norm, seq_len, seq_per_batch):
...
def train(model, features, df_train, df_val, ref_norm, seq_len, seq_per_batch, epochs, checkpoint_path):
    x_train, y_train, x_val, y_val = get_train_data(
        df_train=df_train, df_val=df_val, features=features, ref_norm=ref_norm, seq_len=seq_len, seq_per_batch=seq_per_batch)
...
def predict(model, features, df_test, ref_norm, seq_len, seq_per_batch):
    x_test, y_test, original_len = get_test_data(df_test, features=features, ref_norm=ref_norm, seq_len=seq_len, seq_per_batch=seq_per_batch)
    ...
    y_pred = np.reshape(y_pred, y_test.shape)[:original_len]
