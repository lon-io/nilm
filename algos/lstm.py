...
from algos.keras_batch_gen import BatchGenerator
...
def get_test_data(df_test, feature, ref_norm, seq_len, seq_per_batch):
    ...
    test_data_generator = BatchGenerator(x_test, y_test, seq_per_batch=seq_per_batch, seq_len=seq_len)
...
def train(model, feature, df_train, df_val, ref_norm, seq_len, seq_per_batch, epochs, checkpoint_path):
    ...
    # history = model.fit(train_data_generator.generate(), steps_per_epoch=n_batches,
    #                 epochs=epochs,
    #                 validation_data=valid_data_generator.generate(), callbacks=callbacks)
